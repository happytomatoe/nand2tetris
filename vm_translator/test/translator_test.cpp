...
string stackPush = translator::stripMargin(R"(
        |@0
        |AM=M+1
        |A=A-1
        |M=D
...
void containsExpectedWithoutWhitespaces(const string &actual,
    ...
    if (ind == std::string::npos) {
        auto diff =
            StringDiff::get_diff_multi_line_strings_in_reverse_order(trA, trE);
        FAIL() << "Diff: " << endl << diff;
    }
}
...
TEST(TranslatorTest, MemoryInit) {
    ...
    containsExpectedWithoutWhitespaces(
        Translator({memorySegmentMinMaxAdress, false, true})
            .translate(tokens, file_name),
        memoryInitAsembly);
}
...
TEST(TranslatorTest, PushConstant) {
    // push constant 1
    // TODO: use D=1
    vector<Token> tokens = {Token(Push), Token(Constant), Token(Number, 1),
                            Token(Eof)};
...
class NormalMemorySegmentTest
    : public testing::TestWithParam<pair<TokenType, int> > {};

TEST_P(NormalMemorySegmentTest, Push) {
    // push mem segment 1
    vector<Token> tokens = {Token(Push), Token(GetParam().first),
...
list<pair<TokenType, int> > memorySegmentToSymbolAddress() {
    ...
    for (auto t : normalMemorySegments) {
        result.emplace_back(t, memory::getSymbolAdress(
                                   memory::token_type_to_memory_segment.at(t)));
    }
...
INSTANTIATE_TEST_SUITE_P(
    TranslatorTest, NormalMemorySegmentTest,
    testing::ValuesIn(memorySegmentToSymbolAddress()),
...
TEST(TranslatorTest, PushTemp) {
    // push constant 1
    vector<Token> tokens = {Token(Push), Token(Temp), Token(Number, 2),
                            Token(Eof)};
    auto actual = Translator(config).translate(tokens, file_name);
    auto t = Utils::preprocess(actual);
...
TEST(TranslatorTest, PushTempInvalid) {
    // push constant 1
    vector<Token> tokens = {Token(Push), Token(Temp), Token(Number, 8),
                            Token(Eof)};
    for (auto token : tokens) {
        cout << token << endl;
    }
    ASSERT_THROW(Translator(config).translate(tokens, file_name),
                 AdressOutOfMemorySegmentRange);
}

TEST(TranslatorTest, PushStatic) {
    // push static 1
    vector<Token> tokens = {Token(Push), Token(Static), Token(Number, 2),
                            Token(Eof)};
    for (auto token : tokens) {
        cout << token << endl;
    }
...
TEST(TranslatorTest, PushPointerThis) {
    // push constant 1
    vector<Token> tokens = {Token(Push), Token(Pointer), Token(Number, 0),
                            Token(Eof)};
    auto actual = Translator(config).translate(tokens, file_name);
    auto t = Utils::preprocess(actual);
...
TEST(TranslatorTest, PushPointerThat) {
    // push constant 1
    vector<Token> tokens = {Token(Push), Token(Pointer), Token(Number, 1),
                            Token(Eof)};
    auto actual = Translator(config).translate(tokens, file_name);
    auto t = Utils::preprocess(actual);
...
TEST(TranslatorTest, PushPointerInvalid) {
    // push constant 1
    vector<Token> tokens = {Token(Push), Token(Pointer), Token(Number, 2),
                            Token(Eof)};
    EXPECT_THROW(Translator(config).translate(tokens, file_name),
                 PointerOutOfRangeException);
}
...
TEST(TranslatorTest, PopStatic) {
    auto number = 1;
    vector<Token> tokens = {Token(Push), Token(Constant), Token(Number, number),
...
TEST(TranslatorTest, Add) {
    ...
    auto expected = push_one + push_one + stripMargin(R"(
        |A=A-1
        |M=D+M
    )");
    containsExpectedWithoutWhitespaces(t, expected);
...
TEST(TranslatorTest, Subtract) {
    vector<Token> tokens = {Token(Push),     Token(Constant), Token(Number, 1),
                            Token(Push),     Token(Constant), Token(Number, 1),
                            Token(Subtract), Token(Eof)};
    auto actual = Translator(config).translate(tokens, file_name);
    auto t = Utils::preprocess(actual);

    auto expected = push_one + push_one + R"(
       A=A-1
       D=D-M
       M=-D
    )";
...
TEST(TranslatorTest, NegateInvalid) {
    vector<Token> tokens = {Token(Negate), Token(Eof)};
    ASSERT_THROW(Translator(config).translate(tokens, file_name),
                 InvalidOperation);
}
...
TEST(TranslatorTest, GreaterThan) {
    vector<Token> tokens = {
        Token(Push),     Token(Constant),  Token(Number, 1),   Token(Push),
        Token(Constant), Token(Number, 1), Token(GreaterThan), Token(Eof)};
    auto actual = Translator(config).translate(tokens, file_name);
    auto t = Utils::preprocess(actual);
...
TEST(TranslatorTest, Not) {
    vector<Token> tokens = {Token(Push), Token(Constant), Token(Number, 0),
                            Token(Not), Token(Eof)};
    auto actual = Translator(config).translate(tokens, file_name);
    auto t = Utils::preprocess(actual);
