...
export class VMWriter extends JackParserListener {
    ...
    override enterSubroutineDeclaration = (ctx: SubroutineDeclarationContext) => {
        const name = ctx.subroutineDecWithoutType().subroutineName().IDENTIFIER().getText()
    ...
    override exitLetStatement = (ctx: LetStatementContext) => {
        ...
        if (ctx.varName() != null) {
            const symbol = this.localSymbolTable!.lookup(ctx.varName()!.IDENTIFIER().getText())
            if (symbol == undefined) {
                throw new Error(`Can't find variable ${ctx.varName()!.IDENTIFIER().getText()} in local symbol table`);
            }
